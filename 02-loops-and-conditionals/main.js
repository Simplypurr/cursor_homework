...
while (isNaN(N)) {
    N = parseInt(prompt('Your input is not valid, try again. Type your start range number..(ex.10)', 10), 10);
};

let M = parseInt(prompt('Input your end range number for calculation..'), 10);
while (isNaN(M)) {
    M = parseInt(prompt('Your input is not valid, try again. Type your end range number..(ex.20)', 20), 10);
};
